...
const logFinish = (operation, start) => {
  ...
  console.log(`${finish}: finish processing ${operation} in ${minutes}:${seconds} minutes`);
};
/**
 * Get recently modified companies as 100 companies per page
 */
const processCompanies = async (domain, hubId, q) => {
  const start = logStart('processCompanies');
  const account = domain.integrations.hubspot.accounts.find(account => account.hubId === hubId);
  ...
  while (hasMore) {
    ...
    console.log(`${new Date().getTime()} fetch company batch`);
  ...
  logFinish('processCompanies', start);
  return true;
};
...
const processContacts = async (domain, hubId, q) => {
  const start = logStart('processContacts');
  const account = domain.integrations.hubspot.accounts.find(account => account.hubId === hubId);
  ...
  while (hasMore) {
    ...
    console.log(`${new Date().getTime()} fetch contact batch`);
  ...
  logFinish('processContacts', start);
  return true;
};
...
const createQueue = (domain, actions) => queue(async (action, callback) => {
  ...
  if (actions.length > 2000) {
    console.log(`${new Date().getTime()} inserting actions to database`, { apiKey: domain.apiKey, count: actions.length });
...
const pullDataFromHubspot = async () => {
  ...
  for (const account of domain.integrations.hubspot.accounts) {
    logStart(`start processing account ${account.hubId}`);
    ...
    try {
      ...
      console.log(`${new Date().getTime()} process companies`);
    } catch (err) {
